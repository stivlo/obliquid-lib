...
public class LanguageTypeImpl implements LanguageType {
	 ...
	 */
	private ImmutableMap<String, String> supported = ImmutableMap.of("en", "English", "it",
			"Italiano");
	...
	public final boolean isValid(final String languageString) {
		...
		return supported.containsKey(languageString.toLowerCase());
	}

	/**
	 * Check whether the languageString is one of the supported languages. Alias
	 * for isValid().
	 * 
	 * @param languageString
	 *            the two letters ISO abbreviation.
	...
	/**
	 * Convert the languageString in the language name in the language itself.
	 * Example: it -> Italiano, en -> English.
	 * 
	 * @param locale
	 *            the locale is ignored because each language name is shown in
	 *            the same language.
	 ...
	 */
	@Override
	public final void setDataFromString(final String languageString)
			throws IllegalArgumentException {
