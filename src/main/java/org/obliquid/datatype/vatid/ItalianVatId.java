...
public class ItalianVatId extends VatId {
         ...
         */
        private final boolean hasInvalidCharacterItalian(final String vatId) {
                char aChar;
                if (!vatId.substring(0, 3).equals("IT-")) {
                        setMessage("Prefix must be IT-");
                        return true;
                }
                String vatIdNumberPart = vatId.substring(3);
                for (int i = 0; i < 11; i++) {
        ...
        private final boolean checkItalianVatId(final String vatIdString) {
                int i, aChar, s;
                setMessage("");
                if (hasInvalidLengthItalian(vatIdString)
                                || hasInvalidCharacterItalian(vatIdString)) {
                        return false;
                }
